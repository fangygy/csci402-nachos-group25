...
void Porter(){
	...
	myID = GetMV(porterID,-1);
	SetMV(porterID, GetMV(porterID,-1) + 1,-1);
	ServerRelease(IDLock,-1);

	while(true){
		ServerAcquire(porterLock[myID],-1);
		if(GetMV(porterBaggageLineLength[myID],-1) != 0){
			ServerAcquire(porterBaggageLineLock[myID],-1);
			...
			ServerSignal(porterBaggageLine[myID], porterBaggageLineLock[myID],-1);
			/*wait for passenger to tell me their name :D*/
			ServerWait(porterLine[myID], porterBaggageLineLock[myID],-1);
			two[0] = myID;
			two[1] = GetMV(GetMV(passengerBaggage[myID],-1),-1);
			...
			SetMV(porterBaggageLineLength[myID], GetMV(porterBaggageLineLength[myID],-1) - 1,-1);
			ServerRelease(porterBaggageLineLock[myID],-1);
		}
		else if(GetMV(porterFirstClassBeddingLineLength[myID],-1)!=0){
			ServerAcquire(porterBeddingLineLock[myID],-1);
			...
			/*I process a passenger*/
			ServerSignal(porterFirstClassBeddingLine[myID], porterBeddingLineLock[myID],-1);
			/*wait for passenger to tell me their name C|:-D*/
			ServerWait(porterLine[myID], porterBeddingLineLock[myID],-1);
			two[0] = myID;
			two[1] = GetMV(GetMV(passengerBeddingHolder[myID],-1),-1);
			Print("Porter %d of Train 0 gives bedding to Passenger %d\n",sizeof("Porter %d of Train 0 gives bedding to Passenger %d\n"),two,2);
			SetMV(porterFirstClassBeddingLineLength[myID], GetMV(porterFirstClassBeddingLineLength[myID],-1) - 1,-1);
			SetMV(GetMV(passengerBeddingHasBedding[myID],-1), true,-1);
			ServerSignal(porterLine[myID], porterBeddingLineLock[myID],-1 );
			ServerRelease(porterBeddingLineLock[myID],-1);
		}
		else if(GetMV(porterBeddingLineLength[myID],-1)!=0){
			ServerAcquire(porterBeddingLineLock[myID],-1);
			/*printf("porter%d has %d people in bedding line and is signalling the first to come up\n", myID, porterBeddingLineLength[myID]);*/
			/*I process a passenger*/
			ServerSignal(porterBeddingLine[myID], porterBeddingLineLock[myID],-1);
			/*wait for passenger to tell me their name C|:-D*/
			ServerWait(porterLine[myID], porterBeddingLineLock[myID],-1);
			two[0] = myID;
			two[1] = GetMV(GetMV(passengerBeddingHolder[myID],-1),-1);
			Print("Porter %d of Train 0 gives bedding to Passenger %d\n",sizeof("Porter %d of Train 0 gives bedding to Passenger %d\n"),two,2);
			SetMV(porterBeddingLineLength[myID], GetMV(porterBeddingLineLength[myID],-1) - 1,-1);
			SetMV(GetMV(passengerBeddingHasBedding[myID],-1), true,-1);
			ServerSignal( porterLine[myID], porterBeddingLineLock[myID] ,-1);
			ServerRelease(porterBeddingLineLock[myID],-1);
		}
			...
			SetMV(porterAvailable[myID], true,-1);
			ServerWait(porter[myID], porterLock[myID],-1);
		}
	}
}/*end of porter*/
