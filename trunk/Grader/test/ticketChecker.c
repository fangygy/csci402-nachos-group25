...
void TicketChecker(){
	...
	int four[4];
                ...
                while(GetMV(currentStop,-1) < MAX_STOPS){
                        ServerAcquire(waitingForStopLock,-1); /*TC will try to acquire a lock he owns, but that's OK*/
                        if(GetMV(conductorWaiting,-1)){
                                ServerSignal(waitingForStop, waitingForStopLock,-1);
                       /* printf("the ticket checker is asleep, waiting for the next stop\n");*/
                                ServerWait(waitingForStop, waitingForStopLock,-1);
                        }
                        else{
                                SetMV(ticketCheckerWaiting, true,-1);
                                ServerWait(waitingForStop, waitingForStopLock,-1);
                        }
                        SetMV(ticketCheckerWaiting, false,-1);
                        /*FOR NOW DONT DO THIS waitingForStopLock->ServerRelease();*/


                        /*wakes up all passengers waiting at the current stop*/
                        ServerAcquire(waitingForTrainLock[GetMV(currentStop,-1)],-1);
                        ServerBroadcast(waitingForTrain[GetMV(currentStop,-1)], waitingForTrainLock[GetMV(currentStop,-1)],-1);
                        ...
                        ServerRelease(waitingForTrainLock[GetMV(currentStop,-1)],-1);
                        /*by law, no one can be in line yet, so I am available*/
                        SetMV(ticketCheckerAvailable, true,-1);

                        for(i = 0; i < GetMV(passengersWaitingToBoard[GetMV(currentStop,-1)],-1); i += 1){
                                /*wait for passengers to come up and give me their ticket*/
                                /*printf("ticket checker is waiting for the next passenger\n");*/
                                ServerWait(ticketChecker, ticketCheckerLock,-1);
                                ...
                                if(GetMV(GetMV(ticketToCheckGetOnStop,-1),-1) == GetMV(currentStop,-1)){
                                        SetMV(GetMV(ticketToCheckOkToBoard,-1), true,-1);
                                        /*increment passenger count?*/
                                        two[0] = GetMV(GetMV(ticketToCheckHolder,-1),-1);
					...
					Print("Ticket Checker validates the ticket of passenger %d of Train 0 at stop number %d\n",sizeof("Ticket Checker validates the ticket of passenger %d of Train 0 at stop number %d\n"),two,2);
                                        ...
                                        SetMV(GetMV(ticketToCheckOkToBoard,-1), false,-1);
                                        two[0] = GetMV(GetMV(ticketToCheckHolder,-1),-1);
                                        two[1] = GetMV(currentStop,-1);
					Print("Ticket Checker invalidates the ticket of passenger %d of Train 0 at stop number %d\n",sizeof("Ticket Checker invalidates the ticket of passenger %d of Train 0 at stop number %d\n"),two,2);
                                ...
                                /*can move on*/
                                ServerSignal(ticketCheck, ticketLock,-1);
                                ServerRelease(ticketLock,-1);                          

                                /*move to the next person in line, or wait for someone to come up*/
                                if(GetMV(ticketCheckerLineLength,-1) > 0){

                                        /*signal on ticketCheckerLine?*/
                                        ServerSignal(ticketCheckerLine, ticketCheckerLock,-1);
                        ...
                        ServerSignal(waitingForGo, waitingForGoLock,-1);
