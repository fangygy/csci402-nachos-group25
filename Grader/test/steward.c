...
void Steward(){
	...
	while(true){
		...
		if(Rand()%5){
			...
			/*diningCarRevenueLock->ServerAcquire();*/
			/*waiterRevenueLock->ServerAcquire();*/
			SetMV(diningCarRevenue, GetMV(diningCarRevenue,-1) + GetMV(waiterRevenue,-1),-1);
			SetMV(waiterRevenue, 0,-1);
			...
			/*waiterRevenueLock->ServerRelease();*/
			/*diningCarRevenueLock->ServerRelease();*/
		}
		ServerAcquire(stewardLock,-1);
		if(GetMV(stewardLineLength,-1) != 0){
			/*serve passenger */

			ServerSignal(stewardLine, stewardLock,-1);
			ServerWait(steward, stewardLock,-1);
			/*passenger has set his ticket value*/
			ServerAcquire(stewardGiveWaiterLock,-1);
			ServerRelease(stewardLock,-1);
			ServerAcquire(diningCarCapacityLock,-1);
			if(GetMV(diningCarCapacity,-1) == 0){
				/*printf("steward can't fit any more passengers into the dining car\n");*/
				one[0] = GetMV(GetMV(stewardTicketToCheckHolder,-1),-1);
				...
				SetMV(GetMV(stewardTicketToCheckWaiter,-1), -1,-1);
				SetMV(stewardLineLength, GetMV(stewardLineLength,-1) - 1,-1);
				ServerSignal(stewardGiveWaiter, stewardGiveWaiterLock,-1);
				...
				for(i=0; i<WAITERS;i += 1){
					...
					ServerAcquire(waiterLock[i],-1);
					if(GetMV(waiterOnBreak[i],-1) == true){
						SetMV(waiterOnBreak[i], false,-1);
						SetMV(GetMV(stewardTicketToCheckWaiter,-1),  i,-1);
						ServerSignal(waiter[i], waiterLock[i],-1);
				...
				if(!foundAWaiter){
					...
					ServerAcquire(waiterLock[0],-1);
					min = GetMV(waiterLineLength[0],-1);
					SetMV(GetMV(stewardTicketToCheckWaiter,-1),  0,-1);
					for(i=1; i<WAITERS;i += 1){
						ServerAcquire(waiterLock[i],-1);
						if(GetMV(waiterLineLength[i],-1) < min){
							ServerRelease(waiterLock[GetMV(GetMV(stewardTicketToCheckWaiter,-1),-1)],-1);
							min = GetMV(waiterLineLength[i],-1);
							SetMV(GetMV(stewardTicketToCheckWaiter,-1), i,-1);
						}
						else{
							ServerRelease(waiterLock[i],-1);
						}
					}
				}
				...
				ServerRelease(waiterLock[GetMV(GetMV(stewardTicketToCheckWaiter,-1),-1)],-1);	
				ServerWait(finalConfirmation, stewardLock,-1);
