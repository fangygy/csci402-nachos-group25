...
PostOffice::Send(PacketHeader pktHdr, MailHeader mailHdr, char* data)
{
    char* buffer = new char[MaxPacketSize];	// space to hold concatenated
						// mailHdr + data
	if(!(mailHdr.length <= MaxMailSize)){
		printf("mailHdr.length %d <= MaxMailSize %d\n",mailHdr.length,MaxMailSize);
		printf("Data being sent: %s\n",data);
	}
    ASSERT(mailHdr.length <= MaxMailSize);
    
    if(!(0 <= mailHdr.to && mailHdr.to < numBoxes)){
    	printf("0 <= mailHdr.to %d < numBoxes %d",mailHdr.to,numBoxes);
    	printf("Data being sent: %s\n",data);
    }
    ASSERT(0 <= mailHdr.to && mailHdr.to < numBoxes);
